...
export default function SimpleBackdrop( {open, onClose, name, image, quote} ) {
    ...
    return (
      <Dialog className="relative z-10" open={notiClick} onClose={() => {
        setSfxOn(true);
        ocarinaSfx.pause();
        ocarinaSfx.currentTime = 0;
        onClose;
        ...
        <Transition show={notiClick} appear={true}>
          ...
          >
            <div className="mt-3 pt-10 text-center sm:ml-4 sm:mt-0 sm:text-left flex justify-center items-center py-5">
              <DialogTitle as="h1" 
              transition
              className={`${sfxOn ? "hidden" : "fixed"} text-4xl font-semibold leading-6 text-gray-900`}>
                🏆 New Achievement Unlocked! 🏆 
              </DialogTitle>
            </div>
            <div className="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 flex flex-col justify-center items-center">
              <img className="block mx-auto h-60" src={image} alt="Achievement Symbol" />
              <div className="mt-3 pt-6 text-center sm:ml-4 sm:mt-0 sm:text-left flex justify-center items-center py-5">
              <DialogTitle as="h1" className={`${sfxOn ? "hidden" : "fixed"} text-2xl font-semibold leading-6 text-gray-900`}>
              ✨💫 {name} 💫✨
              </DialogTitle>
            </div>
              <div className={`${sfxOn ? "hidden" : "relative" } ml-4 text-xl italic text-gray-900`}>{quote}</div>
            </div>
          </DialogPanel>
        </Transition>
        </div>
